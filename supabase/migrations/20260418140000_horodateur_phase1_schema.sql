...
-- Recreate direction_terrain_positions on the Phase 1 contract before dropping
-- horodateur_events.user_id (view previously referenced that column).
-- Public user_id = chauffeurs.auth_user_id (employee account), not actor_user_id
-- (which may be a Direction actor on manual corrections).
