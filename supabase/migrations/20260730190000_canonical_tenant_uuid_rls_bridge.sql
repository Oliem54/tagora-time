-- Canonical tenant UUID bridge (Option C) — additive security hardening.
-- RLS authority: organizations.id (uuid) via organization_memberships.
-- Personal rows: tenant = chauffeurs.organization_id (never row.organization_id override).
-- Team rows (chauffeur_id IS NULL): tenant = row.organization_id (nullable until reconciliation).
-- primary_company / company_context / user_metadata are NOT RLS authorities.
-- No environment-specific UUID hardcoding. No staging data backfill.

-- ---------------------------------------------------------------------------
-- A. Bridge column on chauffeurs
-- ---------------------------------------------------------------------------
...
comment on column public.chauffeurs.organization_id is
  'Canonical SaaS tenant UUID (organizations.id). Nullable until controlled reconciliation. RLS authority for chauffeur-linked data. primary_company remains operational company only.';

-- ---------------------------------------------------------------------------
-- A2. Team-tenant columns on sales_objectives / commission_entries
-- ---------------------------------------------------------------------------
