...
type CreateVehiculeUnavailabilityBody = {
  vehicule_id?: unknown;
  start_at?: unknown;
  end_at?: unknown;
  reason?: unknown;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as CreateVehiculeUnavailabilityBody;
    const vehiculeId = Number(body.vehicule_id);
    ...
    if (!Number.isFinite(vehiculeId) || vehiculeId <= 0) {
      return NextResponse.json({ error: "Vehicule invalide." }, { status: 400 });
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Erreur serveur lors de la creation de l'indisponibilite vehicule.",
      },
      { status: 500 }
    );
  }
