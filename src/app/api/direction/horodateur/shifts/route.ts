...
import { buildHorodateurPastShifts } from "@/app/lib/horodateur-v1/past-shifts-service.server";
import type {
  ...
  try {
    ...
    if (!startParsed.ok || !endParsed.ok) {
      return buildHorodateurValidationErrorResponse({
        ...
        code: "invalid_dates",
        route: "/api/direction/horodateur/shifts",
      });
    }
    ...
    const payload = await buildHorodateurPastShifts({
