...
export default function DirectionPayrollPage() {
  ...
  const filteredRows = useMemo(() => {
    return rows
      ...
      .sort((left, right) => {
        const rightDate = right.last_work_date ?? right.first_work_date ?? "";
        const leftDate = left.last_work_date ?? left.first_work_date ?? "";
  ...
  const companyTotals = useMemo<CompanyTotals[]>(() => {
    ...
    filteredRows.forEach((row) => {
      ...
      buckets.set(row.company_context, {
        ...
        totalBillable: toNumber(row.total_billable),
      });
    });

    return ACCOUNT_REQUEST_COMPANIES.map((company) => buckets.get(company.value)).filter(
      (item): item is CompanyTotals => Boolean(item)
    );
  }, [filteredRows]);
      ...
      <section className="tagora-panel" style={{ marginTop: 24 }}>
        ...
        >
          <div>
            ...
            <p className="tagora-note">
              Tri par date de fin de periode la plus recente.
            </p>
          </div>
