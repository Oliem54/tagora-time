...
describe("access admin effective-role wiring", () => {
  ...
  it("does not modify AuthGate for this remediation", () => {
    const authGate = readFileSync(
      join(root, "src/app/components/AuthGate.tsx"),
      "utf8"
    );
    // Baseline membership bypass remains; no new finance inventiveness required here.
  ...
  it("documents that client org id is not trusted for authorization", () => {
    ...
    const photos = readFileSync(
      join(root, "src/app/lib/storage/photos-dossiers-org.server.ts"),
      "utf8"
    );
    expect(membershipShared).toContain(
      "Never trusts client-supplied organization_id"
    );
    expect(photos).toContain("client_org_rejected");
  });
});
