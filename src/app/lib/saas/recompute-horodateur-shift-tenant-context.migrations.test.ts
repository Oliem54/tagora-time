...
const SUFFIX = "_recompute_horodateur_shift_tenant_context.sql";
const PREVIOUS =
  "20260823220951_recompute_horodateur_current_state_tenant_context.sql";
...
describe("recompute_horodateur_shift tenant-context SQL contract", () => {
  it("ships the official CLI migration after the current-state tenant file", () => {
  ...
  it("replaces only recompute_horodateur_shift and keeps its signature", () => {
    expect(bodies).toMatch(
      /create or replace function public\.recompute_horodateur_shift\s*\(\s*p_employee_id\s+bigint\s*,\s*p_work_date\s+date\s*\)\s*returns void/
    );
    expect(bodies.match(/create or replace function/g)).toEqual([
      "create or replace function",
    ]);
    expect(bodies).not.toContain("trg_horodateur_events_recompute_shift");
    expect(bodies).not.toContain("trg_recompute_horodateur_shift");
    expect(bodies).not.toContain("recompute_horodateur_current_state");
  ...
  it("adds tenant fields to the shift INSERT and ON CONFLICT update", () => {
    expect(bodies).toMatch(
      /insert into public\.horodateur_shifts\s*\([\s\S]*organization_id[\s\S]*organization_company_id[\s\S]*\)/
    );
    ...
    expect(bodies).toMatch(/on conflict \(employee_id, work_date\)/);
  ...
  it("preserves shift calculation, refused-event filter, and the empty-agg delete path", () => {
    ...
    expect(bodies).toContain("'ouvert'::public.horodateur_shift_status");
    expect(bodies).toContain("'ferme'::public.horodateur_shift_status");
    expect(bodies).toContain(
      "e.status <> 'refuse'::public.horodateur_event_status"
    );
    expect(bodies).toContain("delete from public.horodateur_shifts s");
    expect(bodies).toContain("and not exists (select 1 from agg)");
  });
